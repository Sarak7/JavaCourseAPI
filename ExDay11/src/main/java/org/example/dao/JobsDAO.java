...
public class JobsDAO {
    ...
    public void insertJob(Jobs jobs) throws SQLException, ClassNotFoundException {
        Class.forName("org.sqlite.JDBC");
        Connection conn = DriverManager.getConnection(URL);
        PreparedStatement st = conn.prepareStatement(INSERT_JOB);
        st.setInt(1, jobs.getJob_id());
        st.setString(2, jobs.getJob_title());
        st.setDouble(3, jobs.getMin_salary());
        st.setDouble(4, jobs.getMax_salary());
        st.executeUpdate();
    }

    public void updateJob(Jobs jobs) throws SQLException, ClassNotFoundException {
        Class.forName("org.sqlite.JDBC");
        Connection conn = DriverManager.getConnection(URL);
        PreparedStatement st = conn.prepareStatement(UPDATE_JOB);


        st.setInt(4, jobs.getJob_id());
        st.setString(1, jobs.getJob_title());
        st.setDouble(2, jobs.getMin_salary());
        st.setDouble(3, jobs.getMax_salary());
        st.executeUpdate();
    }
